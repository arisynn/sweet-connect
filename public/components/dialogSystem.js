...
window.isGlobalDialogOpen = false;

window.Dialog = {
    _isOpen: false,
    _lastCb: null,
    showSuccess: (title, desc, buttonText = 'OK') => {
        if (!window.Dialog._isOpen) { window.Dialog._lastCb = (f) => window.Dialog._internalClose(f); window.PopupManager.register(window.Dialog._lastCb); window.Dialog._isOpen = true; }
    ...
    close: () => {
        if (window.Dialog._isOpen) {
            window.PopupManager.unregister(window.Dialog._lastCb, false);
